...
| 应用步骤模板 | 3 | 模板中心「应用模板」/ 实验详情「调用」/ 实验列表「使用模板创建」 |
...
### D9 72 个硬编码 hex 值 + 令牌分散两处

软件有 4 套主题 + 暗色模式，任何硬编码颜色切主题必穿帮。令牌又分散在 `app/static/css/app.css` 的 `:root` 和 `app/static/css/themes.css` 两个文件，`--surface-soft` 等只在 `themes.css` 的 `html` 上定义。
