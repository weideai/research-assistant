...
version="$($python_bin -c 'from version_info import APP_VERSION; print(APP_VERSION)')"
...
sed "s/^Version: .*/Version: $version/" "$project_root/packaging/linux/control" > "$package_root/DEBIAN/control"
...
if command -v dpkg-deb >/dev/null 2>&1; then
  dpkg-deb --root-owner-group --build "$package_root" "$dist_root/research-assistant_${version}_amd64.deb"
else
  echo "dpkg-deb is unavailable; skipping .deb output." >&2
fi

tar -C "$app_dist" -czf "$dist_root/research-assistant_${version}_linux_amd64.tar.gz" ResearchAssistant
