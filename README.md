...
- 单个实验可导出为 UTF-8 Markdown 报告，包含实验信息、步骤清单和全部记录
...
数据库会在首次运行时自动创建于 `instance/research.db`。备份时应同时保存这个数据库、`instance/secret_key`、`instance/credential_key` 和 `instance/uploads/` 中的自定义背景。
...
实验详情页的“导出报告”会下载一个 UTF-8 Markdown 文件。文件按以下顺序组织：

1. 实验名称、编号、状态、负责人和计划日期。
2. 实验目的。
3. 按步骤顺序排列的完成状态、执行人、计划/完成日期和步骤说明。
4. 按日期正序排列的实验条件、过程、结果和结论备注。

Markdown 能保留“一项实验对应多个步骤和多条记录”的层级结构，也能在 VS Code、Typora 和常见笔记软件中打开并继续编辑。CSV 更适合后续增加“全部实验索引”和统计数据批量导出，不作为单个实验完整档案的主格式。
